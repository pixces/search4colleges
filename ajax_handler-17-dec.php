					...
					<tr>
						...
						<td width='75%'>";
							...
							elseif($key == 'city'){
								...
								$getstate = get_record_sql('SELECT * FROM '.$CFG->prefix.'city WHERE status="active" and state_id = "'.$personal_info->state.'"');
								...
								//echo "<input type='text' name='$key' id= '$key' value='".$value."' />";
								echo '<input type="text" name="'.$key.'" id= "'.$key.'" value="'.$value.'" class="validate[\'required\']"/>';
							}
						echo"</td>
					</tr>";
			}
			echo '<tr><td colspan="3" align="center" ><a href="javascript:void(0);" onclick="checkPersionalDetails();" ><img src="images/submit.jpg" /></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="populate_personal_detail();" ><img src="images/cancel_bottom.jpg" /></a></td></tr>';
	...
	if(isset($_POST['flag']) && $_POST['flag']=='populate_personal_detail') {
		...
		if($personal_info){
			...
			foreach($personal_info as $key=>$value){
				...
				echo"
					<tr>
					<td width='22%'><b><div align='right'>".$title ."</div></b></td>
					<td width='75%'>";
...
$sections = '';
	...
	if(isset($_POST['flag']) && $_POST['flag']=='editAcademic') {
		...
		echo"<form name='academic_form' id='academic_form' method='post'>	<table  border='0' cellspacing='10' cellpadding='2'>		
				<tr>
					<th width='200px'>Institute Name</th>
		...
		if($personal_info){			
			...
			foreach($personal_info as $record){
				...
				echo"
					...
					<td><input type='text' name='degree_change' id= 'degree_change' value='$record->degree'  /></td>
					<td ><input readonly='true' name='year_of_passing_change' id='year_of_passing_change' type='text' class='input-text2  date  demo_vista1' style='width:150px;' value='$record->year_of_passing'></td>";
	...
	if(isset($_POST['flag']) && $_POST['flag']=='populateAcademic') {
		...
		echo "<tr><td colspan='4'><a href='javascript:void(0);' onclick='showAcademic()'>ADD</a></td></tr>";
		echo "</table";
	}
	...
	if(isset($_POST['flag']) && $_POST['flag']=='showAcademic') {
		...
		echo"
			...
			<td><input type='text' name='degree_add' id= 'degree_add' value=''  class=\"validate['required']\" /></td>
...
?>
		...
		<tr>		
			<td align="right" colspan="3"><input type="image" name="submit" src="images/submit.jpg" />&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' onclick='populateAbout();' ><img src="images/cancel_bottom.jpg" /></a><!-- <a href='javascript:void(0);' onclick='updateAbout();' >Save</a> --></td>
		</tr>
		<tr>
		...
		<tr>
			<td valign='top'><b>Goals description</b></td>
			<td>&nbsp;&nbsp;&nbsp;</td>
			<td>
		...
		<tr>
			<td valign='top'><b>Expectation description</b></td>
			<td>&nbsp;&nbsp;&nbsp;</td>
			<td>
...
<?php	
	...
	if(isset($_POST['flag']) && $_POST['flag']=='populateAbout') {
		...
		if($studeny_info){
			if($table =='student'){
				...
				if(!empty($short_description) || !empty($long_description) || !empty($goals_in_life) || !empty($expectation_from_s4c)) 
					$show =1;
			}else{
		...
		if($studeny_info){
			...
			if($show == 0){
				...
				echo"<div style='text-align:left;'><a href='javascript:void(0);' onclick='$edit_function();' >Add</a><div>";	
			}else{
				$edit_function = ($table =='student')?'editAbout':'editAboutOther';
				...
				if($table =='student'){
						...
						if((!empty($studeny_info->long_description)) &&($studeny_info->long_description !='<br />')){
							echo"<b>Long Description  :</b> <p> $studeny_info->long_description </p>";
						}				
						?><div style="width:100%; float:left;"><?php  ?> </div><?php
						if((!empty($studeny_info->goals_in_life)) &&($studeny_info->goals_in_life !='<br />')){
							echo"<b>Goal in Life  :</b> <p> $studeny_info->goals_in_life </p>";
						}
						?><div style="width:100%; float:left;"><?php  ?> </div><?php
						if((!empty($studeny_info->expectation_from_s4)) &&($studeny_info->expectation_from_s4c !='<br />')){
				 ...
				 if($id == $record->id){
					...
					<div class='achievements_content' id='achievements_content_$record->id'>  
						...
						<tr>
							...
							<td><input readonly='true' name='year' id='year' type='text' class='input-text2  date  demo_vista1' style='width:150px;' value='<?php echo $record->year; ?>'></td>
						</tr>
	...
	if(isset($_POST['flag']) && $_POST['flag']=='addAchievements') {
		...
		?>
					...
					<div class='achievements_content' id='achievements_content'>  
						...
						<tr>
							<td><strong>Year</strong></td>
							<td><input readonly='true' name='year' id='year' type='text' class='input-text2  date  demo_vista1' style='width:150px;' value='' class='validate["required"]'></td>
						</tr>

						<tr>
							<td valign='top'><strong>Short Description</strong></td>
							<td><textarea  name='short_description' id='short_description'  cols='30' rows='4' class='validate["required"]'></textarea></td>
						</tr>

						<tr>
							<td><strong>Add Image</strong></td>
							<td><input name='myfile' type='file'   size='28' class='validate["required","image"]'/></td>
						</tr>
		...
		if($studeny_info){
			if($show == 0){
				...
				echo "  <div class='in_Right_link'><a href='javascript:void(0);' onclick='editProfessional();' ><img src='images/edit.gif' border='0' /></a></div>";
					$work_from = !empty($studeny_info->work_from)?date('M Y',$studeny_info->work_from):'';
					$work_till = !empty($studeny_info->work_till)?date('M Y',$studeny_info->work_till):'';
					echo"
	...
	if(isset($_POST['flag']) && $_POST['flag']=='editProfessional') {
		...
		if($studeny_info){
			...
			echo "<div class='in_Right_link'><input type='image' name='submit' src='images/submit.jpg' /><a href='javascript:void(0);' onclick='populateProfessional();' ><img src='images/cancel_bottom.jpg'></a></div>";
			foreach($studeny_info as $record){
				/*
				echo"
					<strong>Organization</strong>
                ...
                <strong>Designation</strong>
				<p><input name='designation' id='designation' value='<?php echo $record->designation; ?>' class="validate['required']"/></p>
				</div>
                </div>
                ...
                <div class="professional_experience_box_small">	
                <strong>Working From</strong>
				<p><input readonly='true' name='work_from' id='work_from' type='text' class='input-text2  date  demo_vista1' style='width:150px;' value='<?php echo $record->work_from; ?>'></p>
				</div>
                <div class="professional_experience_box_small_right">
                <strong>Working Till</strong>
				<p><input readonly='true' name='work_till' id='work_till' type='text' class='input-text2  date  demo_vista1' style='width:150px;' value='<?php echo $record->work_till; ?>'></p>
				</div>
                </div>
                ...
                <strong>Nature Of Work</strong>
				<p><textarea name='work_nature' id='work_nature' class="validate['required']"><?php echo $record->work_nature; ?></textarea></p>
				</div>
                <div class="professional_experience_box">	
                <strong>Short Description</strong>
				<p><textarea name='short_description' id='short_description' class="validate['required']"><?php echo $record->short_description; ?></textarea></p>
				</div>
				<?php
	...
	if(isset($_POST['flag']) && $_POST['flag']=='search_friends') {
		...
		if($search_list){
			...
			foreach($search_list as $record){ 
				...
				echo "							 
					<div class='$friend_cetogery'> <a rel='lightbox[img]' href='".$imageb."' title='".$record->first_name."'><img src='".$image."'/></a>
...
OP;
	...
	if(isset($_POST['flag']) && $_POST['flag']=='membership') {
		...
		$schoolsql = "select * from {$CFG->prefix}school_membership sm INNER JOIN {$CFG->prefix}school_member_ship_types mt on(sm.school_memberShip_typeid = mt.id) where sm.school_id  = $school_id AND sm.status = 'active'";
			...
			if($friend_list){
				foreach($friend_list as $record){	
					...
					if($friend_info->user_type != 'school')
					{
						...
						{
							$image = "file.php/personal/f3_".$friend_info->image;
					...
					echo"
						 <div class='friend_cetogery'> <a rel='lightbox[img]' href='".$imageb."' title='".$name."'><img src='".$image."' /></a>
	...
	if(isset($_POST['flag']) && $_POST['flag']=='populate_friend_req') {	
		...
		if($frnd_req_pending){
			...
			foreach($frnd_req_pending as $record){
				...
				$friend_name	= ($friend_info->user_type == 'school')?$friend_info->school_name:$friend_info->first_name;

				echo "<h3>$i ) Friend Request : $friend_name </h3> ";
