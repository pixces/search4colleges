			...
			print  '<div class="calendar_box">
						<div class="month_name_box">
							<div class="month_name_left_arrow">
								<img src="images/callander_left_arrow.png" style="cursor:pointer" alt="Previus" width="17" height="17" onclick="showPrevMonth();" />
							</div>
							<div class="month_name">'.date("F Y", mktime(0, 0, 0, $this->month,0,$this->year)).'</div>
							<div class="month_name_right_arrow">
								<img src="images/callander_right_arrow.png" style="cursor:pointer" alt="Next" width="17" height="17" onclick="showNextMonth();" />
							</div>
						</div>
						<div class="callendar_days">';
							for($i=0;$i<7;$i++){
								echo '<div class="'.$this->wday_names[$i].'">'.$this->wday_names[$i].'</div>';
							}
			print	'</div>';
						...
						for($i=1;$i<=$no_days;$i++)
						{
								if($i == $this->today)
								{
									print '<div class="callendar_dates_current"><span class="callendar_numbers">'.$i.'</span></div>';
								}
								else
								{
									print '<div class="callendar_dates"><span class="callendar_numbers">'.$i.'</span></div>';
								}
								$rec++;
						}
