...
<!--  fb:login-button v="2" size="medium">Sign in with Facebook</fb:login-button  -->
</div>
<script>
  // Additional JS functions here
  window.fbAsyncInit = function() {
    FB.init({
      appId      : '404249572981333'; //'189109157855116'--akter, //'404249572981333'--for search, // App ID
	  ...
	  if (response.status === 'connected') {
			   ...
			   FB.api('/me', function(response) {
				...
				/*		
					...
					alert(response.name);
			window.location = "http://www.search4colleges.com/fb_register_complete.htm?name="+ response.name + "&firstname=" + response.first_name +" " + response.middle_name + "&lastname=" + response.last_name + "&dob=" + response.birthday+ "&address=" + response.location.name+ "&gender=" + response.gender
