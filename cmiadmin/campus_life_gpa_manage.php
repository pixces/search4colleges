<?php 
	...
	$decription='';
	$message	= "";
	$master_table_name = 'school_culture_campuslife_gpa';	
	...
	$perpage		= 10;      
	$txtsearch_gpa	= optional_param('txtsearch_gpa', '', PARAM_RAW);
	...
	$columns = array('id','gpa','');
	$column = get_sorting_header($page_arr,$page_name,$theme,$columns);
	extract($column);

	$sort_term		= $sort." ".$dir;				
	$searchfields	= array("gpa");
	$extraselect	= " status != 'delete' ";
	if($txtsearch_gpa!='')
	{
		$extraselect .= " AND gpa LIKE '%".$txtsearch_gpa."%'";
	}

	$master_data = get_cmi_records($master_table_name, true, '*', $txtsearch_gpa, $searchfields, $extraselect, $sort_term, $page, $perpage);
	$master_count = get_cmi_records($master_table_name, false, '', $txtsearch_gpa, $searchfields, $extraselect);
									...
									<td class="Label" align="center" width="90%">
										<table>
											<tr>
												<td>Title&nbsp;:&nbsp; </td>
												<td>
													<input type="text" size="40" name="txtsearch_gpa" id="txtsearch_gpa" class="txtBox" value="<?php echo $txtsearch_gpa; ?>">
												</td>
											</tr>											
											<tr>
												<td colspan="2">
													<center>
														...
														<input name="btnreset" id="btnreset" type="button" value="Reset" onclick="txtsearch_gpa.value=''">
