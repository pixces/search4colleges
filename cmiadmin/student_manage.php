<?php 
	...
	if(isset($_GET) && isset($_GET['delete']))
	{
		...
		if(isset($student_data->user_id) && $student_data->user_id != ''){
			...
			$message_sql = "select * from ".$CFG->prefix."messages where receiver_id = ".$student_data->user_id." OR sender_id = ".$student_data->user_id;
			...
			$academic_delete = "DELETE  from ".$CFG->prefix."academic where student_id = ".$student_data->user_id;
			execute_sql($academic_delete,false);
			
			/* Delete from achievement */
			$achievement_delete = "DELETE  from ".$CFG->prefix."achievement where user_id = ".$student_data->user_id;
...
//			execute_sql($blog_comment_delete,false);
			...
			/* Delete from friends */
		   $friends_delete = "DELETE  from ".$CFG->prefix."friends where (user_id = ".$student_data->user_id." || friend_id = ".$student_data->user_id.")";
			execute_sql($friends_delete,false);
			
			/* Delete from student */
			$student_delete = "DELETE  from ".$CFG->prefix."student where user_id = ".$student_data->user_id;
			execute_sql($student_delete,false);
			
			/* Delete from fe_users */
			 $fe_users_delete = "DELETE  from ".$CFG->prefix."fe_users where id = ".$student_data->user_id;
...
//			execute_sql($blog_comment_delete,false);
	...
	if(isset($_GET) && isset($_GET['active_inactive']))
	{
		...
		if($user_id){
			...
			cmi_add_to_log('student',$status,$status.'d student id: '.$user_id);
	...
	if(isset($_GET) && isset($_GET['inactive']) || isset($_GET['active']))
	{
		...
		foreach($idexploded as $value){
			...
			if(update_record('student', $delete_record)){
				//$status = str_replace('e','a',$status);
				$fe_id=get_field('student','user_id','id',$value);
				$delete_record->id		= $fe_id;
				update_record('fe_users', $delete_record);
				
				cmi_add_to_log('student',$status,$status.'d student id: '.$value);
				$message = "student Status Updated Successfully !!";		
			}
		}
	}	
   ...
   $columns = array("check",'id', 'first_name','date_of_birth', 'educational_interest', 'address', 'goals_in_life', 'expectation_from_s4c');
    ...
    foreach ($columns as $column) {
		...
		if($column != 'check'){
			$$column = "<a href=\"student_manage.php?sort=$column&amp;dir=$columndir&amp;search=$search\">".$string[$column]."</a>$columnicon";
		}
		else{
	...
	$user_data = get_cmi_records('student', true, '*', $search, $searchfields, $extraselect, $sort_term, $page, $perpage);
	$user_count = get_cmi_records('student', false, '', $search, $searchfields, $extraselect);
...
<?php 
	...
	?>
		<script type="text/javascript">
			...
			{
				call_alert_confirm("Are you sure you want to delete this record?",'student_manage.php?delete='+val);
			...
			function confirm_deleteall(val)
			{
				...
				if(total != '')
					call_alert_confirm("Are you sure you want to delete selected Event?",'student_manage.php?deleteall='+tot);
				else{
			...
			function active_inactive(val)
			{
				...
				tot = total.substring(0,total.length-1);
				if(total != '')
					call_alert_confirm("Are you sure you want to mark selected records "+val+"?","student_manage.php?"+val+"="+tot);
				else{
             ...
             <td class="Label" align="center" width="90%">
                ...
                <table>
					...
					<tr>
						...
						<td><input name="btnReset" id="btnReset" type="reset" value="Clear Search" onClick="location.href='student_manage.php'"></td>
					</tr>
				</table>
			</td>
             <td width="5%"></td>
             ...
             <td align="center" width="90%">
				...
				<?php
					if (!empty($table)) {
						...
						echo '<div align="left">
								<a onclick="confirm_deleteall(); return false;" href="student_manage.php?deleteall=1">Delete All</a>
									<a id="active" onclick="active_inactive(this.id); return false;" href="student_manage.php?statusall=active"><img src="theme/'.$theme.'/images/add1.png" alt="active"/></a>
									<a id="inactive" onclick="active_inactive(this.id); return false;" href="student_manage.php?statusall=inactive"><img src="theme/'.$theme.'/images/delete1.png" alt="active"/></a>
								</div>';
						print_paging_bar($user_count, $page, $perpage,
										 "student_manage.php?sort=$sort&amp;dir=$dir&amp;search=$search&amp;".$searchdescription."perpage=$perpage&amp;");
					}
					else
					{	
