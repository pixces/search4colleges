<?php 
	...
	$decription='';
	$message	= "";
	$txtsearch_club_title='';
	$txtsearch_type='';
	$master_table_name = 'school_culture_campuslife_clubs';	
	...
	$perpage		= 10;      
	$txtsearch_club_title	= optional_param('txtsearch_club_title', '', PARAM_RAW);
	...
	$page_arr		= array(
							...
							'perpage'		=> $perpage,
							'page'			=> $page,
							'txtsearch_club_title'=> $txtsearch_club_title
						);	
	...
	$columns = array('id','club_title','');
	$column = get_sorting_header($page_arr,$page_name,$theme,$columns);
	extract($column);

	$sort_term		= $sort." ".$dir;				
	$searchfields	= array("club_title");
	$extraselect	= " status != 'delete' ";
	if($txtsearch_club_title!='')
	{
		$extraselect .= " AND club_title LIKE '%".$txtsearch_club_title."%'";
	}
	$master_data = get_cmi_records($master_table_name, true, '*', $txtsearch_club_title, $searchfields, $extraselect, $sort_term, $page, $perpage);
	$master_count = get_cmi_records($master_table_name, false, '', $txtsearch_club_title, $searchfields, $extraselect);
	...
	{
		$table->head = array ($id,$club_title,"");
									...
									<td class="Label" align="center" width="90%">
										<table>
											<tr>
												<td>Title&nbsp;:&nbsp; </td>
												<td>
													<input type="text" size="40" name="txtsearch_club_title" id="txtsearch_club_title" class="txtBox" value="<?php echo $txtsearch_club_title; ?>">
												</td>
											</tr>											
											<tr>
												<td colspan="2">
													<center>
														...
														<input name="btnreset" id="btnreset" type="button" value="Reset" onclick="txtsearch_club_title.value=''">
