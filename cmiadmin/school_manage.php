...
if (isset($_GET) && isset($_GET['delete'])) {
    ...
    if (isset($school_data->user_id) && $school_data->user_id != '') {
        ...
        // Get fe_user info
        $user_info = get_record('fe_users', 'id', $school_data->user_id);
...
if (isset($_GET) && isset($_GET['featured'])) {
    ...
    //based on the value of featured variable
    //mark and unmark schools as featured
    //Multiple schools can be featured
    //at a given period
    ...
    if ($_GET['featured'] == 'featured'){
        $update_mode->featured = 'featured';
    } else {
        $update_mode->featured = 'no';
    }
    ...
    /*
    //remove this since multiple schools can be featured
    //at the same time
...
/** update recommended */
if (isset($_GET) && isset($_GET['recommend'])) {
    $id = required_param('id', PARAM_INT);

    $update_mode = new object();
    $update_mode->id = $id;
    if ($_GET['recommend'] == 'recommend'){
        $update_mode->recommend = 'yes';
    } else {
        $update_mode->recommend = 'no';
    }
    ...
    //check and remove all previous recommendations
    $sql = "select * from {$CFG->prefix}schools where status='active' and recommend = 'yes' and id != $id";
    $check_recommend = get_records_sql($sql);
    if (($check_recommend)) {
        foreach ($check_recommend as $records) {
            $add_mode = new object();
            $add_mode->id = $records->id;
            $add_mode->recommend = 'no';
            update_record('schools', $add_mode);
        }
    }
}
...
if (isset($_GET) && isset($_GET['statistics'])) {
    $id = required_param('id', PARAM_INT);

    $update_mode = new object();
    $update_mode->id = $id;
    $update_mode->statistics = $_GET['statistics'];
...
if (!$user_data) {
     ...
     */
     ...
     foreach ($user_data as $data) {
        ...
        $recommend = 'recommend';
        $recommend_image = 'thumb_down.png';
        if ($data->recommend == 'yes'){
            $recommend = 'do not Recommend';
            $recommend_image = 'thumb_up.png';
        }

        $statistics = 'Show Statistics';
        $statValue = 1;
        $statistics_image = 'chart-off.png';
        if ($data->statistics == '1'){
            $statistics = 'Hide Statistics';
            $statValue = 0;
            $statistics_image = 'chart-on.png';
        }
                        ...
                        <td class="Label" align="center" width="90%">
                            ...
                            <table>
                                <tr>
                                    <td>Name&nbsp;:&nbsp; </td>
                                    <td>
                                        <input name="txtsearch_name" id="txtsearch_name" class="txtBox"
                                               value="<?php echo $txtsearch_name;?>" type="text">
                                    </td>
                                </tr>
                                <tr>
                                    <td>Address&nbsp;:&nbsp; </td>
                                    <td>
                                        <input name="txtsearch_address" id="txtsearch_address" class="txtBox"
                                               value="<?php echo $txtsearch_address;?>" type="text">
                                    </td>
                                </tr>
                                <tr>
                                    <td><input name="btnSearch" id="btnSearch" type="submit" value="Search"></td>
                                    <td><input name="btnReset" id="btnReset" type="reset" value="Clear Search"
                                               onClick="location.href='school_manage.php'"></td>
                                </tr>
                            </table>
                        </td>
                        <td width="5%"></td>
