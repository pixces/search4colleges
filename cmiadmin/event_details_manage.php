<?php 
	...
	$sort			= optional_param('sort', 'opening_date', PARAM_TEXT);
	...
	$columns = array('id', 'name','short_description');
	...
	{
		$table->head = array ($id, $name,"<a href='javascript: void(0)'>Duration</a>",$short_description,"");
		$table->align = array ("center", "center", "center","center","centerss");
		$table->width = "100%";
		foreach ($master_data as $data)
		{
			...
			$editbutton = get_buttons("Edit $page_title", "$page_form?edit_load=$data->id","theme/$theme/images/edit.gif", "rel='lightbox[edit_$data->id 500 300]'");
			...
			$table->data[] = array (
					"$data->id",
					"$data->name",
					"From : ".date('d/m/Y',$data->opening_date)." To : ".date('d/m/Y',$data->closing_date),
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0">
		...
		<tr>
			<td>
				...
				<a href="<?php echo $page_form; ?>" style="float:right;margin-right:60px;" rel="lightbox[set2 500 300]" class="tipz"  title=":: Add <?php echo $page_title;?>">
											...
											<tr>
												<td colspan="2">
													<center>
														...
														<input name="btnreset" id="btnreset" type="button" value="Reset" onclick="txtsearch_name.value='',txtsearch_description.value=''">
