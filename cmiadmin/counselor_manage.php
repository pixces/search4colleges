<?php 
	...
	if(isset($_GET) && isset($_GET['delete']))
	{
		...
		if(isset($counselor_data->user_id) && $counselor_data->user_id != ''){
			...
			$gallery_data = get_records('gallery','user_id',$counselor_data->user_id);
			...
			$message_sql = "select * from ".$CFG->prefix."messages where receiver_id = ".$counselor_data->user_id." OR sender_id = ".$counselor_data->user_id;
			...
			// Delete from Counselor Query 
				
			$counselor_sql = "select * from ".$CFG->prefix."counselor_query where counselor_id = ".$counselor_data->user_id." OR user_id = ".$counselor_data->user_id;

			$counselor_qry = get_records_sql($counselor_sql); 

			if(!empty($counselor_qry)){
				foreach($counselor_qry as $counsellor){
					
					$counsellor_delete = "DELETE  from ".$CFG->prefix."counselor_query where id = ".$counsellor->id;
					execute_sql($counsellor_delete,false);
				}
			}
			
			/* Delete from academic */
			$academic_delete = "DELETE  from ".$CFG->prefix."academic where student_id = ".$counselor_data->user_id;
			execute_sql($academic_delete,false);
			
			/* Delete from achievement */
			$achievement_delete = "DELETE  from ".$CFG->prefix."achievement where user_id = ".$counselor_data->user_id;
			execute_sql($achievement_delete,false);
			
			/* Delete from blog */
			$blog_delete = "DELETE  from ".$CFG->prefix."blog where user_id = ".$counselor_data->user_id;
			...
			/* Delete from friends */
		   $friends_delete = "DELETE  from ".$CFG->prefix."friends where (user_id = ".$counselor_data->user_id." || friend_id = ".$counselor_data->user_id.")";
			execute_sql($friends_delete,false);
			
			/* Delete from counselors */
			$student_delete = "DELETE  from ".$CFG->prefix."counselors where user_id = ".$counselor_data->user_id;
			execute_sql($student_delete,false);
			
			/* Delete from articles */
			 $articles_delete = "DELETE  from ".$CFG->prefix."articles where user_id = ".$counselor_data->user_id;
			 execute_sql($articles_delete,false);
			 
			 
			/* Delete from fe_users */
			 $fe_users_delete = "DELETE  from ".$CFG->prefix."fe_users where id = ".$counselor_data->user_id;
			 execute_sql($fe_users_delete,false);
			 $message = " counselors Deleted Successfully !!";	
	...
	if(isset($_GET) && isset($_GET['active_inactive']))
	{
		...
		if(update_record('counselors', $record)){
		
			$fe_id=get_field('counselors','user_id','id',$user_id);
			$record->id		= $fe_id;
			update_record('fe_users', $record);
			//$status = str_replace('e','a',$status);
			cmi_add_to_log('counselors',$status,$status.'d counselors id: '.$user_id);
			$message = "counselors Status Updated Successfully !!";
		}
	}
	if(isset($_GET) && isset($_GET['inactive']) || isset($_GET['active']))
	{
		...
		foreach($idexploded as $value){
			...
			if(update_record('counselors', $delete_record)){
				$fe_id=get_field('counselors','user_id','id',$value);
				$delete_record->id		= $fe_id;
				update_record('fe_users', $delete_record);
				//$status = str_replace('e','a',$status);
				cmi_add_to_log('counselors',$status,$status.'d counselors id: '.$value);
				$message = "counselors Status Updated Successfully !!";		
			}
		}
	}	
   ...
   $columns = array("check",'id', 'first_name','date_of_birth', 'educational_interest', 'address', 'goals_in_life', 'expectation_from_s4c');
    ...
    foreach ($columns as $column) {
		...
		if($column != 'check'){
			$$column = "<a href=\"counselor_manage.php?sort=$column&amp;dir=$columndir&amp;search=$search\">".$string[$column]."</a>$columnicon";
		}
		else{
	...
	$user_data = get_cmi_records('counselors', true, '*', $search, $searchfields, $extraselect, $sort_term, $page, $perpage);
	$user_count = get_cmi_records('counselors', false, '', $search, $searchfields, $extraselect);
	 ...
	 if (!$user_data) {
		  ...
		  $table->width = "100%";
				...
				$disablebutton = get_buttons('Active/inactive',"counselor_manage.php?active_inactive=$data->id&status=$status","theme/$theme/images/$image");

				$deletebutton = get_buttons('Delete',"#","theme/$theme/images/delete.png","onclick=\"confirm_delete($data->id); return false;\"");					
				
				$counselors = get_record('fe_users','id',$data->user_id);
				$email = (isset($counselors->email))?$counselors->email:'';
...
<?php 
	...
	?>
		<script type="text/javascript">
			...
			{
				call_alert_confirm("Are you sure you want to delete this record?",'counselor_manage.php?delete='+val);
			...
			function confirm_deleteall(val)
			{
				...
				if(total != '')
					call_alert_confirm("Are you sure you want to delete selected Event?",'counselor_manage.php?deleteall='+tot);
				else{
			...
			function active_inactive(val)
			{
				...
				tot = total.substring(0,total.length-1);
				if(total != '')
					call_alert_confirm("Are you sure you want to mark selected records "+val+"?","counselor_manage.php?"+val+"="+tot);
				else{
             ...
             <td class="Label" align="center" width="90%">
                ...
                <table>
					...
					<tr>
						...
						<td><input name="btnReset" id="btnReset" type="reset" value="Clear Search" onClick="location.href='counselor_manage.php'"></td>
					</tr>
				</table>
			</td>
             <td width="5%"></td>
             ...
             <td align="center" width="90%">
				...
				<?php
					if (!empty($table)) {
						...
						echo '<div align="left">
								<a onclick="confirm_deleteall(); return false;" href="counselor_manage.php?deleteall=1">Delete All</a>
									<a id="active" onclick="active_inactive(this.id); return false;" href="counselor_manage.php?statusall=active"><img src="theme/'.$theme.'/images/add1.png" alt="active"/></a>
									<a id="inactive" onclick="active_inactive(this.id); return false;" href="counselor_manage.php?statusall=inactive"><img src="theme/'.$theme.'/images/delete1.png" alt="active"/></a>
								</div>';
						print_paging_bar($user_count, $page, $perpage,
										 "counselor_manage.php?sort=$sort&amp;dir=$dir&amp;search=$search&amp;".$searchdescription."perpage=$perpage&amp;");
					}
					else
					{	
