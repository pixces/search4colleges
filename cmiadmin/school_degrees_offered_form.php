...
<?php
	...
	{			
		...
		$add_mode	 = new object();
		$add_mode->degree = $degree;
		$add_mode->added_date = $added_date ;
	
		if($new = insert_record($master_table_name, $add_mode))
		{
			cmi_add_to_log('school degrees offered ','Insert','New school degrees offered '.$degree.' added');
?>	
			<script  type="text/javascript">
				parent.window.location = '<?php echo $page_manage; ?>';
				parent.Mediabox.close();			
				...
				<form method="post" name="frm_add_mode" id="frm_add_mode" action="<?php echo $page_name; ?>" enctype="multipart/form-data">
					<table border="0" width="100%" align="center" cellspacing="1" cellpadding="1">
						<tbody>
							...
							<tr>
								<td class="Label" width="20%"><span style="font-size: 12px; font-family : Verdana;">Degree</span> : </td>
								<td >
									<input name="degree" class="validate['required'] txtBox" type="text" size="40" value="<?php echo $degree; ?>" />
								</td>
							</tr>	
							<tr>
								<td colspan="2"><br></td>												
