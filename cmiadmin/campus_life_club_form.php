<?php 
	...
	$club_title = optional_param('club_title','', PARAM_TEXT);	
	...
	if(isset($_GET) && isset($_GET['edit_load']))
	{
		...
		$club_title				= $edit_data->club_title;		
	}
	else
	{
		$edit_id = 0;
	...
	if(isset($_POST['update_mode']))
	{
		...
		$update_mode->club_title = $club_title;		
...
<?php
	...
	{			
		...
		$add_mode	 = new object();
		$add_mode->club_title = $club_title;
		$add_mode->added_date = $added_date ;
	
		if($new = insert_record($master_table_name, $add_mode))
		{
			cmi_add_to_log('Campus Life Qpa ','Insert','New campuslife_qpa '.$club_title.' added');
?>	
			<script  type="text/javascript">
				parent.window.location = '<?php echo $page_manage; ?>';
				parent.Mediabox.close();			
				...
				<form method="post" name="frm_add_mode" id="frm_add_mode" action="<?php echo $page_name; ?>" enctype="multipart/form-data">
					<table border="0" width="100%" align="center" cellspacing="1" cellpadding="1">
						<tbody>
							...
							<tr>
								<td class="Label" width="20%"><span style="font-size: 12px; font-family : Verdana;">Title</span> : </td>
								<td >
									<input name="club_title" class="validate['required'] txtBox" type="text" size="40" value="<?php echo $club_title; ?>" />
								</td>
							</tr>	
							<tr>
								<td colspan="2"><br></td>												
