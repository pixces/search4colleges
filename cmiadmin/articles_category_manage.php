<?php 
	...
	{
		$table->head = array ($id,$name,$sort_order,"");
		...
		foreach ($master_data as $data)
		{
			...
			$editbutton = get_buttons("Edit $page_title" ,"$page_form?edit_load=$data->id","theme/$theme/images/edit.gif", "rel='lightbox[edit_$data->id 500 230]'");
			...
			if($data->parent_id!=0)
			{
				$name=get_field('article_category','name','id',$data->parent_id);
				$name .= " -> $data->name";
			}
			else
			{
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0">
		...
		<tr>
			<td>
				...
				<a href="<?php echo $page_form; ?>" style="float:right;margin-right:60px;" rel="lightbox[add 500 230]" class="tipz"  title=":: Add <?php echo $page_title; ?>">
