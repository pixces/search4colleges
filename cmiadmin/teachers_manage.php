<?php 
	...
	if(isset($_GET) && isset($_GET['delete']))
	{
		...
		$teachers_data 	= get_record('teacher','id',$delete_id);

		if(isset($teachers_data->user_id) && $teachers_data->user_id != ''){
            /* Delete profile image, photos and videos uploaded by user */
            // Get fe_user info
            $user_info = get_record('fe_users','id',$teachers_data->user_id);
            delete_images($user_info->image,'personal',true);
			

			// Delete from Gallery 

			$gallery_data = get_records('gallery','user_id',$teachers_data->user_id);
			...
			$message_sql = "select * from ".$CFG->prefix."messages where receiver_id = ".$teachers_data->user_id." OR sender_id = ".$teachers_data->user_id;
			...
			$academic_delete = "DELETE  from ".$CFG->prefix."academic where student_id = ".$teachers_data->user_id;
			execute_sql($academic_delete,false);
			
			/* Delete from achievement */
			$achievement_delete = "DELETE  from ".$CFG->prefix."achievement where user_id = ".$teachers_data->user_id;
			execute_sql($achievement_delete,false);
			
			/* Delete from blog */
			$blog_delete = "DELETE  from ".$CFG->prefix."blog where user_id = ".$teachers_data->user_id;
			execute_sql($blog_delete,false);
		
			/* Delete from blog_comment */
//			$blog_comment_delete = "DELETE  from ".$CFG->prefix."blog_comment where user_id = ".$teachers_data->user_id;
//			execute_sql($blog_comment_delete,false);

			/* Delete from save_search */
			$save_search_delete = "DELETE  from ".$CFG->prefix."save_search where user_id = ".$teachers_data->user_id;
			execute_sql($save_search_delete,false);
			
			/* Delete from friends */
		   $friends_delete = "DELETE  from ".$CFG->prefix."friends where (user_id = ".$teachers_data->user_id." || friend_id = ".$teachers_data->user_id.")";
			execute_sql($friends_delete,false);
			
			/* Delete from teacher */
			$student_delete = "DELETE  from ".$CFG->prefix."teacher where user_id = ".$teachers_data->user_id;
			execute_sql($student_delete,false);
			
			/* Delete from fe_users */
			 $fe_users_delete = "DELETE  from ".$CFG->prefix."fe_users where id = ".$teachers_data->user_id;
...
//			execute_sql($blog_comment_delete,false);
	...
	if(isset($_GET) && isset($_GET['active_inactive']))
	{
		...
		if(update_record('teacher', $record)){
		    
			$fe_id=get_field('teacher','user_id','id',$user_id);
			$record->id		= $fe_id;
			update_record('fe_users', $record);
			//$status = str_replace('e','a',$status);
			cmi_add_to_log('teacher',$status,$status.'d teacher id: '.$user_id);
	...
	if(isset($_GET) && isset($_GET['inactive']) || isset($_GET['active']))
	{
		...
		foreach($idexploded as $value){
			...
			if(update_record('teacher', $delete_record)){
			    
				$fe_id=get_field('teacher','user_id','id',$value);
				$delete_record->id		= $fe_id;
				update_record('fe_users', $delete_record);
				//$status = str_replace('e','a',$status);
				cmi_add_to_log('teacher',$status,$status.'d teacher id: '.$value);
				$message = "teacher Status Updated Successfully !!";		
			}
		}
	}	
   ...
   $columns = array("check",'id', 'first_name','date_of_birth', 'educational_interest', 'address', 'goals_in_life', 'expectation_from_s4c');
    ...
    foreach ($columns as $column) {
		...
		if($column != 'check'){
			$$column = "<a href=\"teachers_manage.php?sort=$column&amp;dir=$columndir&amp;search=$search\">".$string[$column]."</a>$columnicon";
		}
		else{
	...
	$user_data = get_cmi_records('teacher', true, '*', $search, $searchfields, $extraselect, $sort_term, $page, $perpage);
	$user_count = get_cmi_records('teacher', false, '', $search, $searchfields, $extraselect);
	 ...
	 if (!$user_data) {
		  ...
		  $table->width = "100%";
				...
				$disablebutton = get_buttons('Active/inactive',"teachers_manage.php?active_inactive=$data->id&status=$status","theme/$theme/images/$image");

				$deletebutton = get_buttons('Delete',"#","theme/$theme/images/delete.png","onclick=\"confirm_delete($data->id); return false;\"");					
				
				$teacher = get_record('fe_users','id',$data->user_id);
				...
				$table->data[] = array ("<input type=\"checkbox\" class=\"check-me\" id=\"deletecheck\" name=\"checkbox[]\" 	value=\"".$data->id."\" >",
					...
					$data->date_of_birth,
					"$data->address",
					"<a href='teacher_detail.php?id=".$data->id."'>View Detail</a>",
					"<a href='teacher_detail.php?id=".$data->id."&action=edit'>Edit Detail</a>",
...
<?php 
	...
	?>
		<script type="text/javascript">
			...
			{
				call_alert_confirm("Are you sure you want to delete this record?",'teachers_manage.php?delete='+val);
			...
			function confirm_deleteall(val)
			{
				...
				if(total != '')
					call_alert_confirm("Are you sure you want to delete selected Event?",'teachers_manage.php?deleteall='+tot);
				else{
			...
			function active_inactive(val)
			{
				...
				tot = total.substring(0,total.length-1);
				if(total != '')
					call_alert_confirm("Are you sure you want to mark selected records "+val+"?","teachers_manage.php?"+val+"="+tot);
				else{
             ...
             <td class="Label" align="center" width="90%">
                ...
                <table>
					...
					<tr>
						...
						<td><input name="btnReset" id="btnReset" type="reset" value="Clear Search" onClick="location.href='teachers_manage.php'"></td>
					</tr>
				</table>
			</td>
             <td width="5%"></td>
             ...
             <td align="center" width="90%">
				...
				<?php
					if (!empty($table)) {
						...
						echo '<div align="left">
								<a onclick="confirm_deleteall(); return false;" href="teachers_manage.php?deleteall=1">Delete All</a>
									<a id="active" onclick="active_inactive(this.id); return false;" href="teachers_manage.php?statusall=active"><img src="theme/'.$theme.'/images/add1.png" alt="active"/></a>
									<a id="inactive" onclick="active_inactive(this.id); return false;" href="teachers_manage.php?statusall=inactive"><img src="theme/'.$theme.'/images/delete1.png" alt="active"/></a>
								</div>';
						print_paging_bar($user_count, $page, $perpage,
										 "teachers_manage.php?sort=$sort&amp;dir=$dir&amp;search=$search&amp;".$searchdescription."perpage=$perpage&amp;");
					}
					else
					{	
